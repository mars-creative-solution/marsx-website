// forked from design-sync lib/source-kit.mjs — components/_backup (dead,
// tsconfig-excluded) and components/preview (unapproved, unintegrated
// experiments) share names with live components (Hero, Footer, Nav,
// OrbitLogo); excluded both dirs from the src scan below (see EXCLUDE_DIR_RX).
// Also excludes specific next/link-/next/navigation-dependent files whose
// client runtime can't evaluate outside a real Next.js app (see EXCLUDE_FILE_RX).
//
...
// Repo-specific: dead/unapproved trees mixed into components/ (see fork header).
const EXCLUDE_DIR_RX = /\/(?:_backup|preview)\//;
// Repo-specific: these import next/link or next/navigation, whose client
// runtime reads framework-injected globals that don't exist outside a real
// Next.js app (crashes the whole synthesized entry at module-evaluation
// time, not just when rendered — componentSrcMap nulls only hide a card,
// they don't stop `export *` from evaluating the file). Excluded from the
// src scan entirely so they never enter the entry.
const EXCLUDE_FILE_RX = /\/(?:Button|Nav|Footer|Hero|NasserHero)\.tsx$/;
...
export async function resolvePackage(ctx) {
  ...
  const srcFiles = srcRoot
    ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)).filter((p) => !EXCLUDE_DIR_RX.test(p) && !EXCLUDE_FILE_RX.test(p))
    : [];
  ...
  // Repo-specific: every real component here is `export default function
  // Name() {}` (Next.js convention) — `export * from` (what step 2 wrote)
  // never re-exports `default` per the ES spec, so window.<GLOBAL> ended up
  // empty ([BUNDLE_EXPORT] 24/24 not a component). Now that enrichment has
  // resolved each component's real srcPath, rewrite the synth entry with
  // explicit named re-exports so the default export lands under its real name.
  if (synthEntry) {
    const named = components.filter((c) => c.srcPath);
    writeFileSync(
      entry,
      named.map((c) => `export { default as ${c.name} } from ${JSON.stringify(c.srcPath)};`).join('\n') + '\n',
    );
    console.error(`  [OVERRIDE] rewrote synth entry with ${named.length} explicit default-export re-exports`);
  }
